// ---------------------------------------------------------------------------
// 프레임 + 필터 편집 화면.
// 프레임/필터를 누르면 별도 "적용" 버튼 없이 즉시 미리보기 Canvas에 반영한다.
...
export class EditorScreen {
   ...
   * @param {HTMLElement[]} refs.filterButtons - dataset.filterId 보유
   */
  constructor(refs) {
    this.previewCanvas = refs.previewCanvas;
    this.ctx = this.previewCanvas.getContext("2d");
    this.frameButtons = refs.frameButtons;
    this.filterButtons = refs.filterButtons;
    ...
    this.filterButtons.forEach((btn) => {
      btn.addEventListener("click", () => this.setFilter(btn.dataset.filterId));
    });
  }
  ...
  getFilter() {
    return FILTERS.find((f) => f.id === this.filterId) || FILTERS[0];
  }
  ...
  _updateActiveButtons() {
    ...
    this.filterButtons.forEach((btn) => {
      btn.classList.toggle("active", btn.dataset.filterId === this.filterId);
    });
  }
